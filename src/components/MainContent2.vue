...
<template>
    <div v-if="activeLink === 'Font licensing'">
        <div class="heading-wrapper flex items-center justify-between sticky top-0">
            ...
            <div class="flex gap-4 items-center" v-if="activeTab === 'Confirmed production fonts'">
                <MtTypography v-if="!isConformFontListSubmitted"
                    v-bind="{ tag: 'p', variant: 'body2--medium', color: 'red-babe--500' }">Please verify your confirmed
                    production font list by: 31.07.2024</MtTypography>
                <MtTypography v-if="isConformFontListSubmitted"
                    v-bind="{ tag: 'p', variant: 'body2--medium', color: 'turtle-green--500' }">Confirmed production
                    fonts last submitted on: Today</MtTypography>
                <MtButton @handleclick="submitFontList" v-click-outside="closeSubmitModal"
                    v-bind="{ disabled: this.isConformFontListSubmitted }">Submit production font list</MtButton>
            </div>
        ...
        <div class="content" v-if="activeTab === 'Confirmed production fonts'">
            <div class="search-bar sticky flex justify-between items-center p-4">
    ...
    <div v-else-if="activeLink === 'Companies'">
        <div class="heading-wrapper p-6">
            <MtTypography v-bind="{ tag: 'h2', variant: 'heading6--medium' }">{{ company_name }}</MtTypography>
        </div>
        <div class="heading-wrapper p-6">
        ...
        <div class="card-wrapper flex">
            <div class="card flex flex-col gap-3 flex-1 p-6">
                <div class="flex flex-row gap-4 items-center">
                    <span class="flex items-center justify-center w-12 h-12 p-2 rounded-lg"
                        style="background-color: var(--mt-color--magneton--50);">
                        <MtIconSync size="md" color="magneton--500"></MtIconSync>
                    </span>
                    <div class="flex flex-col">
                        <MtTypography v-bind="{ tag: 'span', variant: 'micro2--regular' }">Last updated: 24/07/2023
                        </MtTypography>
                        <MtTypography v-bind="{ tag: 'h4', variant: 'heading7--medium' }">Sync & Downloads
                        </MtTypography>
                    </div>
                </div>
                <MtTypography class="cursor-pointer"
                    v-bind="{ tag: 'span', variant: 'body2--medium', color: 'blue-duck--500' }">View report
                </MtTypography>
            </div>
            <div class="card flex flex-col gap-3 flex-1 p-6">
                <div class="flex flex-row gap-4 items-center">
                    <span class="flex items-center justify-center w-12 h-12 p-2 rounded-lg"
                        style="background-color: var(--mt-color--taco-yellow--50);">
                        <MtIconTypeList size="md" color="taco-yellow--500"></MtIconTypeList>
                    </span>
                    <div class="flex flex-col">
                        <MtTypography v-bind="{ tag: 'span', variant: 'micro2--regular' }">Last updated: 10/12/2023
                        </MtTypography>
                        <MtTypography v-bind="{ tag: 'h4', variant: 'heading7--medium' }">My library report
                        </MtTypography>
                    </div>
                </div>
                <MtTypography class="cursor-pointer"
            ...
            <div class="card flex flex-col gap-3 flex-1 p-6">
                <div class="flex flex-row gap-4 items-center">
                    <span class="flex items-center justify-center w-12 h-12 p-2 rounded-lg"
                        style="background-color: var(--mt-color--turtle-green--50);">
                        <MtIconTypeWeb size="md" color="turtle-green--500"></MtIconTypeWeb>
                    </span>
                    <div class="flex flex-col">
                        <MtTypography v-bind="{ tag: 'span', variant: 'micro2--regular' }">Last updated: 14/05/2024
                        </MtTypography>
    ...
    <div class="sidebar" v-if="isSidebarOpen" @click.stop :class="isSidebarOpen ? 'open' : 'close'">
        ...
        <div class="sidebar-content" v-if="isInnerSidebarOpen">
            ...
            <div class="sidebar-body h-max">
                <ul class="">
                    <li v-for="(row, index) in HelveticaNowMT" :key="index"
                        class="sidebar-item p-6 border-b cursor-pointer" @click="handleItemClick(index)">
                        <div class="flex gap-2 items-center justify-between mb-2">
                            <div class="flex gap-4 items-center">
                                ...
                                <div class="flex gap-2 items-center checkbox-text">
                                    <MtIconMonotype></MtIconMonotype>
                                    <MtTypography v-bind="{ tag: 'span', variant: 'body2--medium' }">{{ row.label }}
                                    </MtTypography>
                                </div>
                            </div>
            ...
            <div v-if="!showBottomBar"
                class="sidebar-footer w-full flex flex-col gap-3 fixed bottom-0 left-0 px-6 py-4 border-t"
            ...
            <div v-if="showBottomBar"
                class="sidebar-footer w-full flex flex-col gap-3 fixed bottom-0 left-0 px-6 py-6 border-t"
                style="background-color: white;">
                <div class="flex gap-4">
                    <MtButton @click="cancelSelection" class="flex-1" color="darkgrey" variant="ghost">Cancel</MtButton>
                    <MtButton @click="makeSelectedForProduction" v-click-outside="closeModal" class="flex-1">Make
                        Selected for Production</MtButton>
                </div>
            </div>
        </div>
    </div>
    ...
    <div v-if="isModalOpen" class="modal">
        <div v-if="!isFontConfirmed" class="modal-content p-8" @click.stop>
            <span class="close" @click="closeModal">
                <MtIconClose></MtIconClose>
            </span>
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">You are confirming these
                fonts as production fonts.</MtTypography>
            <MtTypography class="mb-5" v-bind="{ tag: 'p', variant: 'body1--regular' }">By doing so, you are validating
                that your organization is using this font as a Production Font under the terms of your licensing
                agreement. If you are not sure what a production font is, please follow this <a href="#">link</a>.
            </MtTypography>
            <div class="flex flex-col gap-3 mb-6">
                <MtTypography v-bind="{ tag: 'p', variant: 'heading7--medium', color: 'spirits--700' }">Add details
                    (optional):</MtTypography>
                <VueTailwindDatepicker class="datepicker" placeholder="Start Date - End Date" separator=" - "
                    v-model="dateValue" :shortcuts="false" :formatter="formatter" />
            </div>
        ...
        <div v-if="isFontConfirmed" class="modal-content p-8" @click.stop>
            ...
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Successfully added!
            </MtTypography>
            <MtTypography class="mb-5" v-bind="{ tag: 'p', variant: 'body1--regular' }">You have successfully added
                production font to your list.</MtTypography>
    ...
    <div v-if="isSubmitModalOpen" class="modal">
        <div v-if="isShowConfirmSubmitModal" class="submit-modal-content p-8" @click.stop>
            <span class="close" @click="closeSubmitModal">
                <MtIconClose></MtIconClose>
            </span>
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">You are about to submit
                the report</MtTypography>
            <MtTypography class="mb-5" v-bind="{ tag: 'p', variant: 'body1--regular' }">This is a summary of all
                Production Fonts that your organization is using. You are contractually obligated to submit this list to
                Monotype each quarter.</MtTypography>
        ...
        <div v-if="!isShowConfirmSubmitModal" class="modal-content p-8" @click.stop>
            ...
            <img :src="successImage" alt="Success" class="mb-5" />
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Successfully submitted!
            </MtTypography>
            <MtTypography class="mb-5" v-bind="{ tag: 'p', variant: 'body1--regular' }">You have successfully submitted
                your quarterly report.</MtTypography>
    ...
    <div v-if="isAcceptFontModalOpen" class="modal">
        <div v-if="isSuccessfullyAdded" class="submit-modal-content p-8" @click.stop>
            <span class="close" @click="closeAcceptModal">
                <MtIconClose></MtIconClose>
            </span>
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Are you sure this is a
                Production Font?</MtTypography>
            <MtTypography class="mb-6" v-bind="{ tag: 'p', variant: 'body1--regular' }">By approving this font, it will
                be added to your Confirmed Production Font list.</MtTypography>
            <div class="flex gap-3">
        ...
        <div v-if="!isSuccessfullyAdded" class="modal-content p-8" @click.stop>
            ...
            <img :src="successImage" alt="Success" class="mb-5" />
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Successfully added font
                as production font.</MtTypography>
            <MtTypography class="mb-5" v-bind="{ tag: 'p', variant: 'body1--regular' }">You have successfully submitted
                your quarterly report.</MtTypography>
            <div class="flex gap-3">
    ...
    <div v-if="isDenyFontModalOpen" class="modal">
        <div v-if="isSuccessfullyRemoved" class="submit-modal-content p-8" @click.stop>
            <span class="close" @click="closeDenyModal">
                <MtIconClose></MtIconClose>
            </span>
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Are you sure this is not
                a Production Font?</MtTypography>
            <MtTypography class="mb-6" v-bind="{ tag: 'p', variant: 'body1--regular' }">By denying this font from the
                list, you are verifying that it should not be considered a Production Font under the terms of your
                license.</MtTypography>
        ...
        <div v-if="!isSuccessfullyRemoved" class="modal-content p-8" @click.stop>
            ...
            <img :src="successImage" alt="Success" class="mb-5" />
            <MtTypography class="mb-6 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Successfully removed
                from the list.</MtTypography>
    ...
    <div v-if="isSwitchTrueModalOpen" class="modal">
        <div v-if="isSwitchTrueActivated" class="submit-modal-content p-8" @click.stop>
            <span class="close" @click="closeSwitchTrueModal">
                <MtIconClose></MtIconClose>
            </span>
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Are you sure this font
                is used in production?</MtTypography>
            <MtTypography class="mb-6" v-bind="{ tag: 'p', variant: 'body1--regular' }">By activating this font, it
                means that this font is used in production. </MtTypography>
        ...
        <div v-if="!isSwitchTrueActivated" class="modal-content p-8" @click.stop>
            ...
            <img :src="successImage" alt="Success" class="mb-5" />
            <MtTypography class="mb-6 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Successfully activated
                font as production font.</MtTypography>
    ...
    <div v-if="isSwitchFalseModalOpen" class="modal">
        <div v-if="isSwitchFalseActivated" class="submit-modal-content p-8" @click.stop>
            <span class="close" @click="closeSwitchFalseModal">
                <MtIconClose></MtIconClose>
            </span>
            <MtTypography class="mb-5 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Are you sure this font
                is not used in production?</MtTypography>
            <MtTypography class="mb-6" v-bind="{ tag: 'p', variant: 'body1--regular' }">If you are deactivating this
                font, it means that this font is not used currently in production.</MtTypography>
        ...
        <div v-if="!isSwitchFalseActivated" class="modal-content p-8" @click.stop>
            ...
            <img :src="successImage" alt="Success" class="mb-5" />
            <MtTypography class="mb-6 pr-20" v-bind="{ tag: 'p', variant: 'heading5--medium' }">Successfully deactivated
                font as production font.</MtTypography>
